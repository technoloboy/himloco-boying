...
Mirrors HIMLoco Go1 obs layout exactly:
  - Actor history (proprio_history): 45D × 6 = 270D
      command(3,×[2,2,0.25]) + ang_vel(3,×0.25) + gravity(3)
    + dof_pos(12) + dof_vel(12,×0.05) + actions(12) = 45D
  - Critic (privileged): 238D = 45D + lin_vel(3,×2.0) + ext_force(3,×1/30) + height(187,×5.0)
...
def _him_observations(cfg: ManagerBasedRlEnvCfg, play: bool) -> dict:
  ...
  Critic is rebuilt explicitly to match HIMLoco 238D:
    45D proprio + lin_vel(3,×2.0) + ext_force(3,×1/30) + height(187,×5.0)
  """
  him_critic_terms = {
    ...
    # ── 3D ext_force (body frame, scale 1/30 to normalize ±30N → ±1.0) ──
  ...
  return {
    "proprio_history": ObservationGroupCfg(
      ...
      nan_policy="warn",  # log NaN occurrences; NaNGuardWrapper handles physics reset
    ),
